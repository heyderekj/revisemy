...
4. Environment variables:
   ...
   - Optional (hosted billing): `PADDLE_CLIENT_SIDE_TOKEN`, `PADDLE_API_KEY`, `PADDLE_WEBHOOK_SECRET`, `PADDLE_PRICE_PRO` (`pri_…` for Pro $9/mo), `PADDLE_SANDBOX=true` until go-live. Cashier webhook: `/paddle/webhook` (enable subscription + transaction notifications in Paddle). Free = 30 credits/mo; Pro = 100. Agents call `create_checkout` → human opens Paddle Checkout.
