...
#[Description('Run a full ReviseMy design checkup: screenshot → review → human pins → apply feedback → optional next pass.')]
...
4. **Hand off** — Share `review_url` with the human. Tell them to pin feedback and approve or request changes.
...
6. **Act on the work packet** — When status changes:
   ...
   - `changes_requested` → apply **human pins first** (`work_packets.pins`, must-fix before nits). Treat `second_opinion` as hints only. Then ship fixes and call `create_review` again with `parent_id` set to this review’s id for pass 2+ (new screenshots of the fixed UI).
7. **Repeat** until `approved` (or the human stops the loop).

## Rules

- Human pins = intent. Second opinion = hints.
